...
| 前缀 | 方向 | 说明 |
|------|------|------|
| `CG_` | 客户�?�?服务�?| 请求消息，客户端直接发送，不需注册处理�?|
| `GC_` | 服务�?�?客户�?| 推送消息，自动生成注册与转发代�?|
| 无前缀 | 内嵌数据 | 不单独走网络（如 `PlayerInfo`�?|

## 消息 ID

ID 定义�?`proto.id` 中（目录下所�?proto 共用）：
...
// �?client/UnityClient/Assets/Scripts/GameLogic/Network/Handlers/GameHandler.cs �?partial 方法中编写业务逻辑
```
